...
export default function TabsBlock({
  ...
  return (
    ...
    >
      {/* Tab headers */}
      <div className={`flex border-b overflow-x-auto ${dark ? 'border-gray-700' : 'border-gray-200'}`}>
        {items.map((item) => (
          <button
            key={item.id}
            onClick={() => setActiveTab(item.id)}
            className={`px-3 sm:px-6 py-2.5 sm:py-3 font-medium transition-colors border-b-2 -mb-px whitespace-nowrap text-sm sm:text-base ${
              activeTab === item.id
