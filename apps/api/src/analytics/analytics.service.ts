...
export class AnalyticsService {
  ...
  async getCourseAnalytics(courseKey: string) {
    ...
    const passRate =
      completedStudents > 0 ? (passedCount / completedStudents) * 100 : 0;

    const allUnitProgress = enrollments.flatMap((e) => e.unitProgress);
    const totalTimeSeconds = allUnitProgress.reduce(
      (sum, up) => sum + up.timeSpentSeconds,
      0,
    );
    const averageTimeSpent =
      completedStudents > 0 ? totalTimeSeconds / completedStudents : 0;

    const averageFocusLoss =
      allUnitProgress.length > 0
        ? allUnitProgress.reduce((sum, up) => sum + up.focusLossCount, 0) /
          allUnitProgress.length
        : 0;

    // Unit breakdown from course output proposedIndex
    const output = course.output as Record<string, unknown> | null;
    const proposedIndex = (output?.proposedIndex as Array<{
      code?: string;
      title?: string;
      units?: Array<{ code?: string; title?: string }>;
    }>) || [];

    const units: Array<{ code: string; title: string }> = [];
    ...
    const unitBreakdown = units.map((unit) => {
      const unitProgressItems = allUnitProgress.filter(
        (up) => up.unitCode === unit.code,
      );
      const completedCount = unitProgressItems.filter(
        (up) => up.completedAt,
      ).length;
      const completionRate =
        totalEnrollments > 0 ? (completedCount / totalEnrollments) * 100 : 0;
      const avgTime =
        unitProgressItems.length > 0
          ? unitProgressItems.reduce(
              (sum, up) => sum + up.timeSpentSeconds,
              0,
            ) / unitProgressItems.length
          : 0;
      const avgFocusLoss =
        unitProgressItems.length > 0
          ? unitProgressItems.reduce(
              (sum, up) => sum + up.focusLossCount,
              0,
            ) / unitProgressItems.length
          : 0;
    ...
    const completionOverTime: Array<{ week: string; count: number }> = [];
    for (let i = 11; i >= 0; i--) {
  ...
  async getCourseUserProgress(courseKey: string) {
    const course = await this.prisma.course.findFirstOrThrow({
      where: { key: courseKey },
    });
    ...
    const result = enrollments
      .map((e) => {
        const completedUnits = e.unitProgress.filter(
          (up) => up.completedAt,
        ).length;
        const progress =
          totalUnits > 0
  ...
  async getOrgAnalytics(orgKey: string) {
  ...
  async getStudentAnalytics(userId: string) {
    ...
    const allUnitProgress = enrollments.flatMap((e) => e.unitProgress);
    const totalTimeSpent = allUnitProgress.reduce(
      (sum, up) => sum + up.timeSpentSeconds,
      0,
    );
    ...
    const courseBreakdown = enrollments.map((e) => {
      ...
      const timeSpent = e.unitProgress.reduce(
        (sum, up) => sum + up.timeSpentSeconds,
        0,
      );
    ...
    const recentActivity = allUnitProgress.slice(0, 10).map((up) => {
      const enrollment = enrollments.find((e) =>
        e.unitProgress.some((p) => p.id === up.id),
      );
      return {
