...
interface FormatToolBarProps {
  ...
  hideControls?: Set<string>;
}

const BORDER_RADIUS_PRESETS = [
  { value: 0, label: '0', radius: '0' },
  { value: 6, label: '20', radius: '3px' },
  { value: 12, label: '50', radius: '5px' },
  { value: 20, label: '70', radius: '7px' },
] as const;
...
export function FormatToolBar({
  ...
  const show = (group: string) => !hideControls?.has(group);

  const containerClass = variant === 'inline'
    ? 'w-full flex flex-wrap justify-center bg-gray-50/80 border-t border-gray-200 rounded-b-xl py-1.5 px-3 gap-0.5'
    : 'inline-flex items-center bg-white rounded-xl shadow-xl border border-gray-200 p-1.5 gap-0.5';
    ...
    <div ref={ref} className={containerClass}>
      ...
      {show('bgColor') && (
        <div className="relative">
          ...
          >
            <PaintBucket className="w-5 h-5" style={{ color: format.bgColor || '#9ca3af' }} />
          </button>
          {openMenu === 'bgColor' && (
            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1.5 z-50">
              <ColorPicker
        ...
        <>
          ...
          {BORDER_RADIUS_PRESETS.map((preset) => (
            <ToolBtn
              ...
              onClick={() => onChange({ ...format, borderRadius: preset.value })}
      ...
      {(show('textColor') || show('bgColor') || show('lineColor')) && (show('hAlign') || show('vAlign') || show('margins')) && (
        <Sep />
      )}
